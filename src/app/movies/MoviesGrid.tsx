...
// Genre name → TMDB genre ID mapping for movies
...
export default function MoviesGrid({ initialMovies }: { initialMovies: any[] }) {
  ...
  const [activeGenre, setActiveGenre] = useState<number | null>(null); // null = All
  ...
  const fetchByGenre = useCallback(async (genreId: number | null, label: string) => {
    ...
    try {
      const res = await fetch(`/api/browse?type=movie&genre=${genreId}&page=1`);
  ...
  // Load more movies (works for both All and genre-filtered)
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/browse?type=movie&category=popular&page=${page}${genreParam}`);
      if (!res.ok) throw new Error('Failed to fetch');
