...
export default function TVGrid({ initialShows }: { initialShows: any[] }) {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/browse?type=tv&category=popular&page=${page}`);
      ...
      if (data.results && data.results.length > 0) {
        const newShows = data.results.map((s: any) => ({
          id: s.tmdbId,
          title: s.title,
          posterPath: s.posterPath?.includes('image.tmdb.org')
            ? s.posterPath.replace('https://image.tmdb.org/t/p/w500', '')
            : s.posterPath,
  ...
  return (
    <div>
      ...
      {/* Load More Button */}
      {hasMore && filteredShows.length > 0 && activeGenre === 'All' && (
